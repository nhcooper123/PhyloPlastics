# Prune trees to match seabird list from Clements 2025 taxonomy
...
# --------------------
seabirds <- read_csv("data-raw/seabird-list-Clements2025.csv")

# Add _ between species names to match tree
seabirds$Clements2025_Binomial <- gsub(" ", "_", seabirds$Clements2025_Binomial)
# -----------------------------------------------------------------
...
# -------------------------------------------------------
# Update taxonomy for some species to match Clements 2025
...
x <- name.check(BBtree, seabirds, data.names = seabirds$Clements2025_Binomial)
x$data_not_tree
...
# ------------------------------------------------
x2 <- name.check(seabird_tree, seabirds, data.names = seabirds$Clements2025_Binomial)
x2$data_not_tree
...
# Write out
write.nexus(seabird_tree, file = "data/seabird-tree-2026.nex")
...
ds1 <- 
  seabirds %>%
  select(Clements2025_Binomial, Clements2025_Family) %>%
  mutate(dup = duplicated(seabirds$Clements2025_Family)) %>%
  filter(dup != TRUE) %>%
  select(Clements2025_Binomial, Clements2025_Family)
...
check <- name.check(phy = seabird_tree, 
                    data = ds1, data.names = ds1$Clements2025_Binomial)
...
ds1 <- ds1[match(familytree$tip.label, ds1$Clements2025_Binomial), ]

# Replace species tips with family names
for(x in 1:length(ds1$Clements2025_Binomial)){
  family <- ds1$Family[x]
...
write.nexus(familytree, file = here("data/family-tree-2026.nex"))