...
# or on laptop
#set_cmdstan_path(path = here("cmdstan"))
...
```{r}
# Read in data
data_noNA <- read_csv(file = here::here("data/plastic-data-aggregated_2025-09-15.csv"))
...
data_noNA_trait <- data_noNA |> 
  unite(col = "diet_type", Carrion:AS.Refuse)
```

### Fit a full model

Note that we need to specify to use brms for bf (`brms::bf`) as it is an internal function in geiger and can cause issues if this package is already loaded.

1. We are modelling this as a binomial response, with number of individuals with plastics and total number of individuals jointly modelled.
2. freeflying has a lot of missing data where it was not specified whether birds were freeflying or nestbound. We don't want to lose all this data so we're going to fit 
...
model1_full_model_bf <- brms::bf(
  n_plastic | trials(n_total) ~ 1 + 
    mi(freeflying) + 
# mi tells brms that is_freeflying contains missing data that should be modelled, not excluded from the model. This parameter is shit - really low ESS. Do we need to include it?
    Date_median + 
    (1 | Source) + 
    (1 | MarineRegion) + 
    (1 + Date_median| Clements2024_Binomial) + 
    (1               | gr(tips, cov = A)) + 
    ...
    Pelagic.Specialist,
  family = "binomial") + 
  brms::bf(freeflying | mi() ~ 1 + (1|Source)) + 
  set_rescor(FALSE)
```
...
foo |> 
  ...
  stat_histinterval() + 
  theme_bw() + 
  labs(y = "Frequency of occurrence of plastic (FO)",
       x = "Marine region")
