...
# Add tips column
data_noNA <- data_noNA %>% mutate(tips = gsub(" ", "_", data_noNA$HBWv8.1_Binomial))
...
model1_full_model_bf <- bf(
  ...
  family = "binomial") + 
  bf(is_freeflying | mi() ~ 1 + (1|Source)) + 
  set_rescor(FALSE)
...
model1_full_model_priors <- c(
  ...
  prior(lkj(2),         class = "cor", group = "tips"),
...
obs_fo <- data_noNA_trait |> 
  group_by(Date_median,TipLabel_HBWBLv6) |> 
...
```{r}
foo <- data_noNA_trait |> 
  modelr::data_grid(ROTL.tip.label = "Raphus cucullatus", 
                    TipLabel_HBWBLv6 = "Raphus cucullatus",
                    body_mass_2sd = 0,#c(-.5, 0, .5),
                    Date_median = 0, 
                    Source = "MacDonald 2024",
